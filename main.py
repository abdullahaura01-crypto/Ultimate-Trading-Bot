...
def main():
    ...
    logger.info(f"  Started at  : {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
    logger.info("=" * 60)
    ...
    def on_ohlc(data):
        ...
        ts = datetime.fromtimestamp(candle["epoch"], tz=timezone.utc).strftime("%H:%M")
        logger.info(
    ...
    # -- Keep alive (ping every 60s - no AuthorizationRequired) ---
    logger.info("Bot running. Press Ctrl+C to stop.")
    try:
        while True:
            time.sleep(60)
            client.ping()   # lightweight ping - no auth errors
